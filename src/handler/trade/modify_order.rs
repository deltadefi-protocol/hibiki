use hibiki_proto::services::{IntentTxResponse, ModifyOrderRequest, OrderType};
...
pub async fn handler(
    ...
    let modify_order_intent = HydraOrderBookIntent::ModifyOrderIntent(Box::new((
        order_datum,
        assets_to_mvalue(&to_hydra_token(&from_proto_amount(
            &authorized_account_value_l1,
        ))),
    )));
    let intent = Box::new((user_account, modify_order_intent));
